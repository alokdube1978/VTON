...
model_path="D:\\VTON\\Models\\selfie_segmenter.tflite"
mp_model_path = 'D:\\VTON\\Models\\pose_landmarker_heavy.task'
...
def getSelfieImageandFaceLandMarkPoints(img,RUN_CV_SELFIE_SEGMENTER=True,use_different_horizontal_vertical_scale=False,force_shoulder_z_alignment=False,use_cv_pose_detector=True):
    ...
    with PoseLandmarker.create_from_options(mp_options) as landmarker:
        ...
        positions.update({
            ...
            "mp_left_ear_z":mp_pose_landmark_list["left ear"][2],
            "mp_right_ear_z":mp_pose_landmark_list["right ear"][2],
            "mp_left_eye_z":mp_pose_landmark_list["left eye"][2],
            "mp_right_eye_z":mp_pose_landmark_list["right eye"][2],
            "mp_left_shoulder_z":mp_pose_landmark_list["left shoulder"][2],
            "mp_right_shoulder_z":mp_pose_landmark_list["right shoulder"][2],
            })
        # positions.update({
        ...
        if (force_shoulder_z_alignment==True and (abs(positions["mp_left_right_shoulder_z_distance"])>normalized_shoulders_z_limit)):
            print("Error!! Z values of left and right shoulder points not aligned:",abs(positions["mp_left_right_shoulder_z_distance"]))
    ...
    eye_nose_slope,eye_nose_intercept=slope_intercept(xy_coordinate_positions["eye_midpoint"],xy_coordinate_positions["nose"])
    eye_line_slope,eye_line_intercept=slope_intercept(xy_coordinate_positions["left_eye"],xy_coordinate_positions["right_eye"])
    xy_coordinate_positions["nose_slope"]=nose_slope
    xy_coordinate_positions["shoulder_slope"]=shoulder_slope
    xy_coordinate_positions["eye_nose_slope"]=eye_nose_slope
    xy_coordinate_positions["eye_line_slope"]=eye_line_slope
    ...
    # if shoulder slope >degrees_shoulder_slope_max
    ...
    print ("Original Eye Nose Slope",file=sys.stderr, flush=True)
    print(math.degrees(math.atan(xy_coordinate_positions["eye_nose_slope"])),file=sys.stderr, flush=True)
    print ("Original Eye Line Slope",file=sys.stderr, flush=True)
    print(math.degrees(math.atan(xy_coordinate_positions["eye_line_slope"])),file=sys.stderr, flush=True)
